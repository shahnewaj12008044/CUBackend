...
export const sendMail = async (
  ...
  auth: {
    user: config.senders_email, // sender's email from config
    pass: config.smtpPass, // SMTP password from config
  },
});
// console.log(config.senders_email, config.smtpPass);

    await transporter.sendMail({
      from: `"COBIANS" <${config.senders_email}>`, // Use the sender's email from config
      to,
      subject,
      text,
      html,
